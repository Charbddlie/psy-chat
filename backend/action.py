import argparse
import pandas as pd
from abc import ABC, abstractmethod
from dataclasses import dataclass
from datetime import datetime
import re
import os
import tomli

class Action(ABC):
    def __init__(self, text):
        self.text = text

    @abstractmethod
    def execute(self):
        """虚函数，需要被子类重写"""
        pass

    @staticmethod
    def format_output(text, chat_dict):
        # 检查文本中是否包含变量标记
        
        # 使用正则表达式找出所有 {变量名} 形式的标记
        variables = re.findall(r'\{(\w+)\}', text)
        
        # 如果存在变量标记
        if variables:
            # 检查所有变量是否都在chat_dict中存在
            missing_vars = [var for var in variables if var not in chat_dict]
            if missing_vars:
                # 如果有缺失变量，抛出错误
                raise ValueError(f"在chat_dict中找不到以下变量: {', '.join(missing_vars)}")
            
            # 所有变量都存在，进行格式化
            return text.format(**chat_dict)
            
        else:
            # 如果没有变量标记，直接打印原文本
            return text

class Say(Action):
    def __init__(self, text):
        super().__init__(text)

    def execute(self, chat_dict):
        print_time = datetime.now()
        text = self.format_output(self.text, chat_dict)
        print(text)
        return rext, {}


class Chat(Action):
    def __init__(self, text, resp_list=['有', '没有'], max_retry=3):
        super().__init__(text)
        self.resp_list = resp_list
        self.max_retry = max_retry

    def execute(self, chat_dict):
        print_time = datetime.now()
        text = self.format_output(self.text, chat_dict)
        print(text)
        for i in range(self.max_retry):
            resp = input(f'请输入{'/'.join(self.resp_list)}')
            if resp in self.resp_list:
                self.resp = resp
                break
            else:
                print(f"输入错误，请重新输入，你还有{self.max_retry-i-1}次机会")

        return text, self.resp


class Exam(Action):
    def __init__(self, text, answer_format='(1.xx 2.xx 3.xx)'):
        super().__init__(text)
        self.answer_format = answer_format

    def execute(self, chat_dict):
        print_time = datetime.now()
        text = self.format_output(self.text, chat_dict)
        print(text)
        answer = input(f'请按以下格式回答：{self.answer_format}')
        return text, answer

class Name(Action):
    def __init__(self, text):
        super().__init__(text)

    def execute(self, chat_dict):
        print_time = datetime.now()
        text = self.format_output(self.text, chat_dict)
        print(text)
        name = input('请直接输入姓名')
        return text, {'name': name}
